...
| 层 | 目录 | 职责 | 关键产出 |
|:---|:---|:---|:---|
| **能力层** | `.claude/skills/` | 11 个 YMOS 能力（含 ymos-core 共享基础设施），每个 skill 自包含 SOP、prompts、knowledge | Agent 发现和执行入口 |
...
**执行具体 Skill/SOP 时再读取**：
- 对应 SKILL.md（`.claude/skills/` 目录下）
- 对应 SOP 文件（`.claude/skills/<skill>/sop.md`）
- 对应 P 提示词（`.claude/skills/<skill>/prompts/` 或 `.claude/skills/ymos-core/prompts/`）
...
| 路径 | 说明 |
|:---|:---|
| `.claude/skills/*/sop.md` | SOP 定义 |
| `.claude/skills/*/prompts/*.md` | P 系列提示词资产 |
| `.claude/skills/ymos-diagnosis/knowledge/**` | 诊断知识库 |
| `.claude/skills/ymos-diagnosis/**` | 投资策略诊断模块 |
| `总入口暗号.md` | 路由表 |
| `AGENT_GUIDE.md` | 本文件 |
| `.env` | API key |
...
```
.claude/skills/ymos-core（共享基础设施）
  → provides: .claude/skills/ymos-core/prompts/ (p2, p9)
  → provides: .claude/skills/ymos-core/templates/ (knowledge-base, memo)
  → provides: .claude/skills/ymos-core/routing.md, watchlist-update-workflow.md

.claude/skills/ymos-market-insight
...
.claude/skills/ymos-strategy
  → reads: data/reports/radar/ (触发来源)
  → reads: data/state/ (状态机 + 个股文件夹全部文件)
  → reads: prompts/ (P3,P5-P8,P10-P12,P17) + .claude/skills/ymos-core/prompts/ (P2,P9)
  → reads: data/state/preferences.md (仓位管理参数，P17使用)
  → writes: data/reports/strategy/ (报告 + 日志)
  → writes: data/state/ (状态机 + 备忘录)

.claude/skills/ymos-research
  → reads: prompts/ (p1, p4) + .claude/skills/ymos-core/prompts/ (p2, p9)
  → writes: data/stocks/*/个股基础知识库.md
  → writes: data/state/*_state.md (P4列)

.claude/skills/ymos-diagnosis（投资策略诊断）
  → reads: .claude/skills/ymos-diagnosis/SKILL.md（核心入口，自包含）
  → reads: knowledge/（可选，深度参考）
  → 不写入任何文件（纯对话式诊断）

.claude/skills/ymos-target-mgmt
  → reads: .claude/skills/ymos-core/templates/ (knowledge-base, memo)
  → calls: .claude/skills/ymos-research (关注时联动)
...
- **大脑**：.claude/skills/（P 系列提示词 + 策略路由 + SOP）— 做决策、出结论
- **眼睛**：cli/ + .claude/skills/ymos-market-insight, ymos-radar（数据抓取 + 市场扫描）— 看市场、报信号
...
2. **处理提示词** → 放入对应 skill 的 `prompts/` 目录，或 `.claude/skills/ymos-core/prompts/`（如为共享）
...
当用户说「诊断一下我的策略」「帮我看看我的投资」「我的投资有什么问题」时，读取 `.claude/skills/ymos-diagnosis/SKILL.md` 启动诊断。诊断模块独立于 YMOS 主流程，不依赖状态机或 P 系列提示词——它有自己的公理体系和诊断框架。
...
```
每日收盘后（四步闭环）：
1. 跑一下市场洞察          → .claude/skills/ymos-market-insight 扫描市场
2. 跑一下投资雷达          → .claude/skills/ymos-radar 桥接报告（趋势+价格+建议）
3. 跑一下策略分析          → .claude/skills/ymos-strategy 处理雷达建议（如有触发）
4. 收口一下                → .claude/skills/ymos-reconcile 一致性校验 + 刷新持仓备忘录视图
