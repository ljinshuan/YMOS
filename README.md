...
│   ├── ymos-core/                 # 共享基础设施（p2, p9, templates, routing）
...
P 系列提示词现在分布在各个 skill 的 `prompts/` 目录中（共享的在 `skills/ymos-core/prompts/`）。它们是**框架样板**——代表一种经过验证的投资决策路由，你完全可以根据自己的投资风格改造它们：调整 P5 FOMO Killer 的评分权重、在 P1 里加入行业特定指标、改变 P12 纪律审查的红线规则、调整 P17 仓位计算器的默认参数。**这才是最核心的进阶——不是接更多数据，而是让系统的大脑越来越像你自己的投资直觉。**
...
能，而且鼓励你改。各 skill `prompts/` 目录下的 P 系列提示词是框架样板，你可以根据自己的投资风格调整权重、修改红线规则、增加行业特定指标、调整仓位参数。**让系统的大脑越来越像你自己的投资直觉，才是最核心的进阶。**
