...
4. **执行策略路由**：
   ...
   | C 持有 | P2 → P6 → [P3] → [P8] → P12 | 读取备忘录原始买入理由；P8 输出须引用辩结论最终评级 |
   | D 卖出 | 读取备忘录 → P2 → P6 → [P10] → P12 | 完全清仓强制 P11 复盘 |
   | E 再平衡 | P17(逐标的) → P7 → P6 → P12 | 先算偏差再做组合决策 |

   > **P12 行为偏误风险评级：** 所有路由（A/B/C/D/E）的 P12 输出现包含 `行为偏误风险`（高/中/低/无）和 `触发偏误` 字段。若评级为 **"高"**，策略报告必须在结论部分醒目提示，并将触发的偏误纳入决策建议依据。

   > **P12 交易行为实证（可选）：** P12 纪律审查可引用 `ymos trade-history fetch` 获取近期实际交易记录，作为行为偏误的客观实证（如频繁换手、追涨杀跌模式）。需 Futu OpenD 在线，不可用时跳过。
